...
var chokidar, consoleMessage, cssExtensions, fs, lastRun, pathlib,
...
module.exports = function(ss, options) {
  var checkEmpty, dir, onChange, takeAction, watchDirs, watcher;
  watchDirs = (function() {
  ...
  onChange = function(path, event) {
    ...
    if (event === 'added' || event === 'changed') {
      return checkEmpty(path, action);
    } else {
      return takeAction(action);
    }
  };
  checkEmpty = function(path, action, checks) {
    if (checks > 6) {
      return takeAction(action);
    } else {
      return fs.readFile(path, function(err, data) {
        if (data.toString().length) {
          ...
          return setTimeout((function() {
            return checkEmpty(path, action, checks + 1);
          }), 250);
        }
      });
    }
  };
  return takeAction = function(action) {
